...
class regviz(object):
    ...
    def set_multipliers(self,multipliers):
        ...
        #Loop through new multipliers dict and check if a feature has previously been set with a multiplier.
        #If it has, reset feature data to original units and update to new feature data. If not, set multiplier to feature.
        if not self.multipliers == None:
            for key,val in zip(multipliers.keys(),multipliers.values()):
                if key in self.multipliers.keys():
                    #Reset to original units
                    self.feature_data.loc[key,'l_ci_95':'h_ci_99'] = self.feature_data.loc[key,'l_ci_95':'h_ci_99']/self.multipliers[key]
                    #Update to new units
                    self.feature_data.loc[key,'l_ci_95':'h_ci_99'] = self.feature_data.loc[key,'l_ci_95':'h_ci_99']*val
        else:
            for key,val in zip(multipliers.keys(),multipliers.values()):
                self.feature_data.loc[key,'l_ci_95':'h_ci_99'] = self.feature_data.loc[key,'l_ci_95':'h_ci_99']*val
    ...
    def undo_multipliers(self):
        ...
        """
        #Check if feature data has been fitted.
        self._check_fitted()
        ...
        #Reset multipliers to None to rid of multipliers that no longer exist.
        self.multipliers = None
    ...
    def plot(self,ax=None,hide_features=None):
        ...
        if not ax == None:
            ...
            if not type(ax) == mpl.axes._axes.Axes:
